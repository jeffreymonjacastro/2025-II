...
int main(int argc, char **argv) {
  int numtasks, rank, next, prev, tag1 = 1, tag2 = 2, N = 1 << 20;
  int sbuf[N], rbuf[N];
  double t0, t1;
  ...
  // 1. Cada proceso envía el buffer a su vecino derecho y recibe de su vecino izquierdo
  MPI_Send(&sbuf, N, MPI_INT, next, tag1, MPI_COMM_WORLD);
  MPI_Recv(&rbuf, N, MPI_INT, prev, tag1, MPI_COMM_WORLD, &status);

  // 2. Utilice send sincrónico, ¿Qué sucede?
  // MPI_Ssend(&sbuf, N, MPI_INT, next, tag1, MPI_COMM_WORLD);
  // MPI_Recv(&rbuf, N, MPI_INT, prev, tag1, MPI_COMM_WORLD, &status);

  // NO-CICLICA
  // 3. Todos los procesos envian menos el ultimo.
